...
# needs data frame with column topic_model consisting of STM models and document-term-matrix in either stm or dfm format
...
# calculate per model indicators
...
# calculate per topic indicators - rows are topics
...
# # model to columns for cosine() calculation, drop everything else
# tidy_mallit_by_term <- tidy_mallit_wider %>% select(-K, -run, -topic, -topic_model, -tidy_model, -top_terms) %>%
#   pivot_longer(!K_run_topic) %>%
#   pivot_wider(names_from=K_run_topic, values_from=value)

# as previous but separately for each K

make_per_K_df <- function(df) {
  df_as_per_list <- df %>%
    select(-K, -run, -topic, -topic_model, -tidy_model, -top_terms) %>%
    pivot_longer(cols=!(one_of("K_run_topic"))) %>%
    pivot_wider(names_from=K_run_topic, values_from=value) %>% 
    split.default(., sub("_.*", "", names(.)))
  tidy_per_K <- tibble(K = names(df_as_per_list[-length(df_as_per_list)]), per_K = df_as_per_list[-length(df_as_per_list)])
  return(tidy_per_K)
}
...
add_cosine_distance <- function(df) {
  df %>% mutate(cosine_distances = map(frames_for_clustering, function(df) 1-cosine(as.matrix(df))))
} 
